package com.kobe.spider.parser;
...
import com.kobe.spider.utility.HtmlUtility;
...
public class JDParser implements Parser {
	 ...
	 // log4j日志记录
    private Logger logger = LoggerFactory.getLogger(JDParser.class);
    ...
    public void parse(Page page) {
        ...
        if (page.getUrl().startsWith("https://item.jd.com/")) {  // 解析商品
            ...
            // 当前页面的商品url列表
            List<String> urls = HtmlUtility.getListUrlByXpath(rootNode, "href", "//div[@id='plist']/ul/li/div/div[1]/a");
            // 下一页 获取下一页的url
            String nextUrl = HtmlUtility.getAttrByXpath(rootNode, "href", "//div[@id='J_topPage']/a[2]");
            ...
            if(System.currentTimeMillis() - start == 0) {   // 解析京东数据页码数时，偶尔获取不到下一页，时间就为0ms，这时需要重试
                ...
                HttpUtility.retryUrl(page.getUrl(), Utility.getTopDomain(page.getUrl()) + Constants.REDIS_KEY_SUFFIX_HIGHER);
            }
        }
    ...
    private void parserProduct(Page page, TagNode tagNode) {
        // 1.id; 商品id
        String id = HtmlUtility.getIdByUrl(page.getUrl());
        page.setId(id);

        // 2.source; 商品来源
        String domain = Utility.getTopDomain(page.getUrl());
        page.setSource(domain);

        // 3.title; 商品标题
        String title = HtmlUtility.getTextByXpath(tagNode, "//div[@class='sku-name']");
        page.setTitle(title);
         ...
         * 当出现这个情况时，就提示更换pduid
         */
        String priceJson = HttpUtility.getPageContent(priceUrl);
        if (priceJson != null) {
        ...
        // 5.imgUrl; 商品图片链接
        String imgUrl = HtmlUtility.getAttrByXpath(tagNode, "data-origin", "//img[@id='spec-img']");
        page.setImgUrl("http:" + imgUrl);

        // 6.params; 商品规格参数
        // Map<String, Map<String, String>>
        // {"主体": {"品牌": Apple}, "型号": "IPhone 7 Plus", "基本信息":{"机身颜色":"玫瑰金"}}
        JSONObject paramObj = HtmlUtility.getParams(tagNode, "//*[@id=\"detail\"]/div[2]/div[2]/div[1]/*", "//h3", "//dl");
        if (paramObj.has("主体")) {
        ...
        String commentCountJson = HttpUtility.getPageContent(commentCountUrl);
